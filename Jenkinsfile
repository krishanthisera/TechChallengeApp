        ...
        stage('Kubernetes Deploy') {
            parallel {
                stage ('Deploying A Brand New Release'){
                    ...
                    steps {
                        container('helm') {
                            sh "helm install --set app.image.repository='${DOCKER_REGISTRY}' --set app.image.tag='${BUILD_NUMBER}' '${RELEASE_NAME}' ./k8s-helm/tech-challenge --create-namespace '${NAMESPACE}'"
                        }
                    }
                }
